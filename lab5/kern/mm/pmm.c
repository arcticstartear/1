...
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
               bool share) {
    // 确保起始地址和结束地址是页面大小的整数倍
    assert(start % PGSIZE == 0 && end % PGSIZE == 0);
    // 确保内存访问范围在用户空间
    assert(USER_ACCESS(start, end));
    // 逐页复制内容
    do {
        // 调用get_pte函数根据地址start找到进程A中对应的页表项（PTE）
        pte_t *ptep = get_pte(from, start, 0), *nptep;
        if (ptep == NULL) {
            // 如果找不到页表项，则向下对齐地址并继续下一轮循环
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue;
        }
        // 调用get_pte函数根据地址start找到进程B中对应的页表项（PTE），如果PTE为空则分配一个新的页表项
        if (*ptep & PTE_V) {
            ...
            assert(npage != NULL);
            int ret = 0;
            /* LAB5:EXERCISE2 2113390
             * 复制page的内容到npage，并建立npage的物理地址和线性地址的映射关系
             * 以下是一些有用的宏和定义，你可以在下面的实现中使用它们。
             * 宏和函数：
